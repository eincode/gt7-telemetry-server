...
function fmtMs(ms: number): string {
  if (ms < 0) return "--:--.---";
  const totalMs = Math.floor(ms); // ← truncate here
...
function bar(fraction: number, width = 28): string {
  const filled = Math.min(
    width,
    Math.round(Math.max(0, Math.min(1, fraction)) * width),
  );
...
export function startSectorTester(opts: SectorTesterOpts): void {
  ...
  function render(): void {
      ...
      for (const lap of lapHistory.slice(-6)) {
        ...
        for (let s = 0; s < sectorCount; s++) {
          ...
          const deltaStr =
            delta != null
              ? delta > 0
                ? C.red(fmtDelta(delta))
                : delta < 0
                  ? C.green(fmtDelta(delta))
                  : C.dim("+0.000s")
              : "   ---   ";
    ...
    out +=
      "\n" +
      C.dim(
        "  Ctrl+C to exit  •  After each lap, enter game sector times when prompted",
      );
  ...
  function promptGameTimes(finishedNum: number, toolSectors: number[]): void {
    ...
    process.stdout.write(
      `  (total ${fmtMs(toolSectors.reduce((a, b) => a + b, 0))})\n\n`,
    );
    ...
    rl.question(
      C.dim(
        "  Game sector times in seconds (e.g. 28.451,15.023,18.204), or Enter to skip: ",
      ),
      (answer) => {
        rl.close();
  ...
  connectOverlay(
    ...
    (msg: StateBroadcast) => {
      ...
      if (!inputMode && currentSector < sectorBoundaries.length) {
        ...
        if (prevSign === null) {
          ...
          const crossingLapT =
            (prevLapT ?? currentLapT) +
            f * (currentLapT - (prevLapT ?? currentLapT));
    ...
    (err) => {
      ...
      process.stderr.write(
        C.dim(`Is the relay server running at ${opts.url}?\n`),
      );
      process.exit(1);
    },
    () => {
